...
const updateStudentProfile = async (req, res) => {
    try {
        ...
        const changes = [
            { field: 'fullName', label: 'Full Name', newValue: fullName },
            { field: 'studentPhone', label: 'Phone Number', newValue: studentPhone },
            { field: 'parentPhone', label: 'Guardian Phone', newValue: parentPhone },
            { field: 'studentEmail', label: 'Student Email', newValue: studentEmail },
            { field: 'parentEmail', label: 'Guardian Email', newValue: parentEmail },
            { field: 'year', label: 'Year', newValue: year },
            { field: 'branch', label: 'Branch', newValue: branch }
        ]
            .filter((change) => change.newValue !== undefined)
            .map((change) => ({
                field: change.field,
                label: change.label,
                oldValue: student[change.field] !== undefined && student[change.field] !== null ? String(student[change.field]) : '',
                newValue: String(change.newValue)
            }))
            .filter((change) => change.newValue.trim() !== '' && change.newValue !== change.oldValue);

        if (changes.length === 0) {
            return res.status(400).json({ message: 'No profile changes detected to submit for approval' });
        }
        ...
        return res.status(201).json({
            message: 'Profile update request submitted and pending warden approval',
