...
// @desc    Get profile change requests for assigned hostel
// @access  Private - Warden only
const getProfileRequestsByWarden = async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'Warden') {
            return res.status(403).json({ message: 'Warden access required' });
        }
        ...
        const students = await Student.find({ hostelBlock: warden.assignedHostel }).select('_id');
        const studentIds = students.map((student) => student._id);

        requestFilter.studentId = { $in: studentIds };

        const requests = await ProfileRequest.find(requestFilter)
            .populate('studentId', 'fullName rollNumber hostelBlock roomNumber studentPhone studentEmail year branch')
            .populate('reviewedBy', 'fullName wardenId')
            .sort({ requestDate: -1 });
...
// @desc    Approve profile change request
// @access  Private - Warden only
const approveProfileRequestByWarden = async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'Warden') {
            return res.status(403).json({ message: 'Warden access required' });
        }
        ...
        const profileRequest = await ProfileRequest.findById(req.params.id).populate('studentId');
        if (!profileRequest) {
            return res.status(404).json({ message: 'Profile request not found' });
        }

        if (profileRequest.status !== 'Pending') {
            return res.status(400).json({ message: `Profile request is already ${profileRequest.status}` });
        }

        const student = profileRequest.studentId;
        if (!student) {
            return res.status(404).json({ message: 'Student not found for this request' });
        }

        if (student.hostelBlock !== warden.assignedHostel) {
            return res.status(403).json({ message: 'You can approve requests only for your assigned hostel students' });
        }
        ...
        profileRequest.changes.forEach((change) => {
            if (allowedFields.has(change.field)) {
                student[change.field] = change.newValue;
            }
        });
        ...
        profileRequest.reviewedBy = warden._id;
        profileRequest.reviewedAt = new Date();
        profileRequest.rejectionReason = undefined;
        await profileRequest.save();

        return res.json({
            message: 'Profile request approved and student profile updated successfully',
...
// @desc    Reject profile change request
// @access  Private - Warden only
const rejectProfileRequestByWarden = async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'Warden') {
            return res.status(403).json({ message: 'Warden access required' });
        }
        ...
        if (student.hostelBlock !== warden.assignedHostel) {
            return res.status(403).json({ message: 'You can reject requests only for your assigned hostel students' });
        }

        profileRequest.status = 'Rejected';
        profileRequest.reviewedBy = warden._id;
        profileRequest.reviewedAt = new Date();
        profileRequest.rejectionReason = (req.body.rejectionReason || '').trim() || 'Rejected by warden';
        await profileRequest.save();
